...
def test_retention_cutoff_is_date_truncated(monkeypatch):
    ...
    # 26 Sep: rejected, duplicate and approved cards all age out; saved stories
    # live on the phone (saved_store.dart), so the sweep never reads `saves`
    ...
    assert all("T00:00:00" in p for p in stories)
    assert not [p for m, p in calls if p.startswith("saves")]
...
def test_ops_blob_content_age_grades_frozen_upstreams():
    ...
    assert ops.blob_content_age_h("participant_oi", {"date": "2026-09-01", "rows": {}}, now) == 36.0
    assert ops.blob_content_age_h("shipping", {"chokepoints": [], "asof": "2026-09-01"}, now) == 36.0
    assert ops.blob_content_age_h("monsoon", {"country": {}, "asof": "2026-09-01"}, now) == 36.0
